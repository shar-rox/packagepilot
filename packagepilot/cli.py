...
# Add parent directory to path so we can import packagepilot modules
...
def format_package_output(package, show_full=False):
    ...
    Args:
        ...
        show_full: If True, show complete details including code example
    """
    output = []
    output.append(f"\n{'='*70}")
    output.append(f"📦 {package.name}")
    output.append(f"{'='*70}")
    output.append(f"\n{package.description}")
...
def cmd_search(args):
    ...
    for i, package in enumerate(results[:5], 1):  # Show top 5
...
def cmd_category(args):
    ...
    print(f"\n💡 Use 'packagepilot info <package-name>' for details")
    db.close()
...
def cmd_categories(args):
    ...
    print("\n💡 Use 'packagepilot category <name>' to see packages")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="packagepilot",
        description="🚀 PackagePilot - Your intelligent Python package discovery assistant",
        epilog="Examples:\n"
    ...
    search_parser.add_argument("query", help="Search query (e.g., 'parse HTML', 'web scraping')")
    ...
    category_parser.add_argument("category_name", help="Category name (e.g., 'web', 'data')")
